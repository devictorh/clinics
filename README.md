...
2. **Stdlib-first com fronteiras hexagonais.** `net/http` puro (ServeMux 1.22+ com métodos e path params), `log/slog`, persistência `map` + `sync.RWMutex` — tudo atrás de ports. Zero dependências externas: o `go.mod` não tem um único `require`. O custo (middlewares e decode manuais) compra transparência total: cada linha do projeto é explicável, e trocar o in-memory por Postgres ou o simulador por um PSP real não toca o core. As invariantes ficam onde conseguem ser atômicas: unicidade de documento dentro do repositório (índice sob o mesmo lock), validações de vínculo no service.
